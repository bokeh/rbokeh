...
rbokeh2gist <- function(fig_str, name, created = NULL, description = "", view = TRUE) {
  ...
  rbokeh2html(p, file = file.path(dir, "index.html"))
  ...
  gst <- gistr::gist_create(files = ff, description = name)

  message("* Browse this gist on github:")
  message("   ", gst$html_url)
  message("* View or share this gist on bl.ocks.org:")
  message("   ", paste0("http://bl.ocks.org/", gst$owner$login, "/", gst$id))
  message("* Embed in an iframe:")
  emb <- paste0("http://bl.ocks.org/", gst$owner$login, "/raw/", gst$id)
  message(paste0('   <iframe width="', p$width + 20, '" height="', p$height + 20, '" frameBorder="0" webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen="" sandbox="allow-forms allow-scripts allow-popups allow-same-origin allow-pointer-lock" src="', emb,'"></iframe>'))
