...
## run this code to fix the issue:
# aa <- readLines("inst/htmlwidgets/lib/bokehjs/bokeh.min.css")
# tmp <- gsub(".*(<svg xmlns.*</svg>).*", "\\1", aa)
# enc <- base64enc::base64encode(charToRaw(tmp))
# tmp2 <- gsub("(.*)(utf8,<svg xmlns.*/svg>#grayscale)(.*)", paste0("\\1base64,", enc, "\\3"), aa)
# cat(tmp2, file = "inst/htmlwidgets/lib/bokehjs/bokeh.min.css")
...
## to fix manually: find something like this in bokeh.min.css:
